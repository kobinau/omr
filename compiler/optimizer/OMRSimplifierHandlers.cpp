...
TR::Node *subSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
              ...
              imulComposer(node, (int *) &MulComposerValue, &MulComposerChild))  // TODO
   {
      ...
      if (lrChild->getOpCode().isLoadConst()) {
         ...
         if ((firstChild->getReferenceCount() == 1) && (performTransformation(s->comp(),
      ...
      case TR::lshl:
         if (firstChild->getSecondChild()->getOpCode().isLoadConst()) {
            if ((firstChild->getSecondChild()->get64bitIntegralValue() & LONG_SHIFT_MASK) < 32) {
               ...
               if (!performTransformation(s->comp(), "%sReducing long operation in node [" POINTER_PRINTF_FORMAT
               "] to an int operation\n", s->optDetailString(), node))
               return false;
...
static TR::Node *intDemoteSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (sourceIs64Bit &&
      ...
      uint32_t childSourceSize = TR::ILOpCode::getSize(
            TR::ILOpCode::getProperConversion(childTargetDataType, childSourceDataType,
                                              wantZeroExtension));  // size of childSourceDataType (hack)
      if (childSourceDataType.isIntegral() && (childSourceSize < sourceSize)) {
         bool wantZeroExtension = childOp.isZeroExtension();
         TR::ILOpCodes foldOp = TR::ILOpCode::getProperConversion(childSourceDataType, targetDataType,
                                                                  wantZeroExtension);
         ...
         for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();) {
            ...
            if (gotoTree) {
               ...
               if (predEndWithGoto || isImmediatePred) {
                  ...
                  TR::SymbolReference *storeSymRef = storeNode->getOpCode().hasSymbolReference()
                                                     ? storeNode->getSymbolReference() : NULL;
                  ...
                  if (storeNode->getOpCode().isStoreDirect() &&
                     ...
                     if (performTransformation(comp, "%sDuplicating %p and insert it after %p\n", s->optDetailString(),
                        ...
                        TR::Block *newIfCmpBlock = TR::Block::createEmptyBlock(block->getEntry()->getNode(), comp,
...
generateDecomposedTree(TR::Node *parentNode, TR::Node *multiplierChild, TR::Simplifier *s, char bitPosition[],
   ...
   if (count <= 2) {
      if (count == 1) {
         if (operationType[from] == OP_MINUS) {
            ...
            if (bitPosition[from] != 0) {
               TR::Node *constNode1 = isLong ? (TR::Node::lconst(parentNode,
         ...
         if (operationType[from + 1] == OP_MINUS) {
            ...
            if (bitPosition[from + 1] == 0) {
               ...
               TR::Node *constNode2 = isLong ? (TR::Node::lconst(parentNode,
      ...
      firstChild = generateDecomposedTree(parentNode, multiplierChild, s, bitPosition, operationType, from, from + mid,
                                          tabspace + 1, isLong);
...
static void decomposeMultiply(TR::Node *node, TR::Simplifier *s, bool isLong) {
   ...
   if ((!s->containingStructure() || s->getLastRun()) &&
      ...
      TR::Node *decomposedMulNode = generateDecomposedTree(node, firstChild, s, bitPosition, operationType, 0, count, 0,
                                                           isLong);
...
TR::Node *getQuotientUsingMagicNumberMultiply(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (node->getOpCodeValue() == TR::idiv || node->getOpCodeValue() == TR::irem) {
      ...
      TR_ASSERT(0,
                "getQuotientUsingMagicNumberMultiply called with incorrect opcode in node %p, need TR::idiv/TR::ldiv/TR::irem/TR::lrem.",
                node);
   }
...
static bool conditionalBranchFold(
      int32_t takeBranch, TR::Node *&node, TR::Node *firstChild, TR::Node *secondChild, TR::Block *block,
      TR::Simplifier *s) {
...
static bool isLegalToMerge(TR::Node *node, TR::Block *block, TR::Block *nextBlock, TR::CFGEdgeList &nextExcpOut,
   ...
   if (!(inEdge.empty()) && (inEdge.front() != outEdge || (inEdge.size() > 1))) {
      for (auto ie = inEdge.begin(); ie != inEdge.end(); ++ie) {
         if ((*ie) != outEdge && (*ie)->getFrom() != cfg->getStart()) {
            ...
            bool isLegal = (*ie)->getFrom()->asBlock()->getLastRealTreeTop()->isLegalToChangeBranchDestination(
                  s->comp());
            if (!isLegal)
               return false;
         }
   ...
   if (block->getNumber() >= 0 && nextBlock->getNumber()) {
      ...
      if (!performTransformation(s->comp(), "%sMerge blocks [" POINTER_PRINTF_FORMAT
      "] and ["
      POINTER_PRINTF_FORMAT
...
static void changeBranchDestinationsForMergeBlocks(TR::Block *block, TR::Block *nextBlock,
                                                   ...
                                                   TR::CFGEdgeList::iterator &inEdgeIter, TR::Simplifier *s) {
...
static bool checkAndReplaceRotation(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   TR::Node *mulNode = firstChild->getOpCode().isMul() ? firstChild : (secondChild->getOpCode().isMul() ? secondChild
                                                                                                        : 0);
   TR::Node *shiftNode = firstChild->getOpCode().isMul() ? secondChild : (secondChild->getOpCode().isMul() ? firstChild
                                                                                                           : 0);

   if (!mulNode || !shiftNode || !(shiftNode->getOpCode().isShiftLogical() && shiftNode->getOpCode().isRightShift()))
      return false;

   TR::Node *mulConstNode = mulNode->getFirstChild()->getOpCode().isLoadConst() ? mulNode->getFirstChild()
                                                                                : (mulNode->getSecondChild()->getOpCode().isLoadConst()
                                                                                   ? mulNode->getSecondChild() : 0);
   TR::Node *shiftConstNode = shiftNode->getFirstChild()->getOpCode().isLoadConst() ? shiftNode->getFirstChild()
                                                                                    : (shiftNode->getSecondChild()->getOpCode().isLoadConst()
                                                                                       ? shiftNode->getSecondChild()
                                                                                       : 0);
   TR::Node *mulNonConstNode = mulNode->getFirstChild()->getOpCode().isLoadConst() ? mulNode->getSecondChild()
                                                                                   : (mulNode->getSecondChild()->getOpCode().isLoadConst()
                                                                                      ? mulNode->getFirstChild() : 0);
   TR::Node *shiftNonConstNode = shiftNode->getFirstChild()->getOpCode().isLoadConst() ? shiftNode->getSecondChild()
                                                                                       : (shiftNode->getSecondChild()->getOpCode().isLoadConst()
   ...
   if ((1 << correctLeftShiftAmount) !=
       mulConst)      // Only a rotate if the const values add up to the size of the node type.  ie for ints, this better add up to 32
      return false;

   if (!performTransformation(s->comp(), "%sReduced or/xor/add in node [" POINTER_PRINTF_FORMAT
   "] to rol\n", s->optDetailString(), node))
   return false;
...
static TR::Node *longToFloatHelper(uint64_t absValue, bool isUnsignedOperation, TR::Node *node, TR::Simplifier *s) {
   ...
   if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 24)) {
      foldFloatConstant(node, isUnsignedOperation ? (float) firstChild->getUnsignedLongInt()
                                                  : (float) firstChild->getLongInt(), s);
      return node;
   } else {
...
static TR::Node *longToDoubleHelper(uint64_t absValue, bool isUnsignedOperation, TR::Node *node, TR::Simplifier *s) {
   ...
   if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 53)) {
      foldDoubleConstant(node, isUnsignedOperation ? (double) firstChild->getUnsignedLongInt()
                                                   : (double) firstChild->getLongInt(), s);
      return node;
...
static void bitwiseToLogical(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (!performTransformation(s->comp(), "%sConvert comparison with bitwise ops [" POINTER_PRINTF_FORMAT
...
TR::Node *indirectLoadSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCodeValue() == TR::loadaddr) {
      ...
      if (isSame &&
          ...
          performTransformation(s->comp(), "%sReplace indirect load %s ["
         POINTER_PRINTF_FORMAT
      "] with ",
   ...
   if (s->comp()->cg()->getSupportsVectorRegisters()) {
      ...
      if ((node->getType().isIntegral() || node->getType().isDouble()) &&
          ...
          performTransformation(s->comp(), "%sReplace indirect load ["
         POINTER_PRINTF_FORMAT
      "] with getvelem", s->optDetailString(), node))
      {
         int32_t offset = 0;
...
TR::Node *indirectStoreSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (isDeletedLabelLoadaddr(secondChild) &&
       performTransformation(s->comp(), "%sRemoving indirect store ["
      POINTER_PRINTF_FORMAT
   "] to deleted label ["
   POINTER_PRINTF_FORMAT
   ...
   if (firstChild->getOpCodeValue() == TR::loadaddr) {
      ...
      if ((storeDataType == addrDataType && isSameType) &&
          ...
          node->getSymbolReference()->getOffset() == 0 &&
          (node->getSymbol()->isVolatile() == firstChild->getSymbol()->isVolatile()) &&
          performTransformation(s->comp(), "%sReplace indirect store %s ["
         POINTER_PRINTF_FORMAT
      "] with ",
            s->optDetailString(), node->getOpCode().getName(), node))
      {
   ...
   if ((node->getOpCodeValue() == TR::awrtbari) && 0 &&
      ...
      if ((valueChild->getOpCodeValue() == TR::New) ||
         ...
         if (removeWrtBar && !s->comp()->getOptions()->realTimeGC() &&
             performTransformation(s->comp(),
                                   "%sFolded indirect write barrier to iastore because GC could not have occurred enough times to require iwrtbar ["
            POINTER_PRINTF_FORMAT
         "]\n", s->optDetailString(), node))
         {
   ...
   if (s->comp()->cg()->getSupportsVectorRegisters()) {
      ...
      if ((node->getType().isIntegral() || node->getType().isDouble()) &&
          ...
          ((addressNode->getOpCode().isArrayRef() && addressNode->getSecondChild()->getOpCode().isLoadConst() &&
            ...
            addressNode->getFirstChild()->getSymbol()->getType().isVector()) ||
           (addressNode->getOpCode().hasSymbolReference() && addressNode->getSymbol()->getType().isVector())) &&
          performTransformation(s->comp(), "%sReplace indirect store ["
         POINTER_PRINTF_FORMAT
      "] with vsetelem", s->optDetailString(), node))
      {
         int32_t offset = 0;
         TR::SymbolReference *oldSymRef = node->getSymbolReference();
...
TR::Node *astoreSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (isDeletedLabelLoadaddr(node->getFirstChild()) &&
       performTransformation(s->comp(), "%sRemoving astore ["
      POINTER_PRINTF_FORMAT
   "] to deleted label ["
   POINTER_PRINTF_FORMAT
...
TR::Node *directStoreSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (child->getOpCode().isLoadVar() &&
       ...
       performTransformation(s->comp(), "%sFolded direct store of load of same symbol on node ["
      POINTER_PRINTF_FORMAT
   "]\n", s->optDetailString(), node))
   {
   ...
   // NOTE: this likely won't improve register pressure; it's purely a codegen quality improvement
      ...
      bool firstLoadsSymRef = (firstGrandchild->getOpCode().isLoadVar() && symRef ==
                                                                           firstGrandchild->getSymbolReference()); // && firstGrandchild->getReferenceCount() > 1);
      bool firstIsLoadConst = (firstGrandchild->getOpCode().isLoadConst());
      bool secondLoadsSymRef = (secondGrandchild->getOpCode().isLoadVar() && symRef ==
                                                                             secondGrandchild->getSymbolReference()); // && secondGrandchild->getReferenceCount() > 1);
      ...
      if ((firstLoadsSymRef && secondIsLoadConst) || (firstIsLoadConst && secondLoadsSymRef)) {
         ...
         if (loadTree->getNode()->getOpCodeValue() == TR::treetop) {
            ...
            if (loadNode->getOpCode().isLoadVarDirect() && loadNode->getSymbolReference() == symRef) {
               ...
               if (insertionTree != NULL && performTransformation(s->comp(), "%smove update tree ["POINTER_PRINTF_FORMAT
               "] to after ["
               POINTER_PRINTF_FORMAT
...
TR::Node *vsetelemSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   for (TR::Node *vsetelemNode = node;
        vsetelemNode && vsetelemNode->getOpCodeValue() == TR::vsetelem; vsetelemNode = vsetelemNode->getFirstChild()) {
...
TR::Node *iaddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (nodeOp == TR::iadd &&
      ...
      if (performTransformation(s->comp(), "%sNormalized iadd of iconst > 0 in node [%s] to isub of -iconst\n",
                                s->optDetailString(), node->getName(s->getDebug()))) {
         TR::Node *newSecondChild = (secondChild->getReferenceCount() == 1) ? secondChild : TR::Node::create(
               secondChild, TR::iconst, 0);
      ...
      if ((lrChild->getOpCodeValue() == TR::iconst) &&
         ...
         if (performTransformation(s->comp(), "%sFound aiadd of iconst with aiadd x and iconst in node [%s]\n",
            ...
            s->_alteredBlock = true;
              ...
              (node->getSecondChild()->getOpCode().isLoadConst()) &&
              // iadd second child should be const to be pushed down
...
TR::Node *laddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst() &&
       performTransformation(s->comp(), "%sSimplified ladd in node ["
      POINTER_PRINTF_FORMAT
   "] to lconst\n", s->optDetailString(), node))
   {
      if (node->nodeRequiresConditionCodes()) {
   ...
   if (node->getOpCodeValue() == TR::ladd &&
       ...
       secondChild->getLongInt() > 0) {
      if (performTransformation(s->comp(), "%sNormalized ladd of lconst > 0 in node [" POINTER_PRINTF_FORMAT
      "] to lsub of -lconst\n", s->optDetailString(), node))
      {
      ...
      {
         if (performTransformation(s->comp(), "%sReduced ladd of -1 and an lneg in node [" POINTER_PRINTF_FORMAT
         ...
         if (performTransformation(s->comp(), "%sReduced ladd with negated first child in node [" POINTER_PRINTF_FORMAT
         "] to lsub\n", s->optDetailString(), node))
         {
            s->anchorChildren(node, s->_curTree);
            TR::Node::recreate(node, TR::lsub);
      ...
      if (performTransformation(s->comp(), "%sReduced ladd with negated second child in node [" POINTER_PRINTF_FORMAT
      "] to lsub\n", s->optDetailString(), node))
      {
         s->anchorChildren(node, s->_curTree);
         TR::Node *newSecondChild = secondChild->getFirstChild();
      ...
      if ((lrChild->getOpCodeValue() == TR::lconst) &&
         ...
         if (performTransformation(s->comp(),
                                   "%sFound aladd of lconst with aladd x and lconst in node ["
            POINTER_PRINTF_FORMAT
         "]\n", s->optDetailString(), node))
         {
            int64_t value = secondChild->getLongInt();
            ...
            s->_alteredBlock = true;
                 ...
                 performTransformation(s->comp(), "%sFound aladd of non-lconst with aladd x and lconst in node ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
   ...
   if (node->getOpCodeValue() != TR::aiadd &&
     ...
     */
      ...
      if (LBase &&RBase && LBase == RBase &&
                           performTransformation(s->comp(), "%sRemove loadaddr in address computation in ["
      POINTER_PRINTF_FORMAT
      "]\n",
            s->optDetailString(), node))
      {
...
TR::Node *lsubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (node->getOpCodeValue() != TR::aladd &&
      ...
      if ((rrChild->getOpCodeValue() == TR::lconst) &&
          ...
          node->cannotOverflow() && secondChild->cannotOverflow()) {
         if (performTransformation(s->comp(),
                                   "%sFolded lsub with children related through lconst in node ["
            POINTER_PRINTF_FORMAT
         "] to lconst \n", s->optDetailString(), node))
         {
            node->setChild(0, 0);
      ...
      // more negative value than positive in a 2's complement signed representation
            ...
            secondChild->getLongInt() > 0) {
      if (performTransformation(s->comp(), "%sNormalized lsub of lconst > 0 in node [" POINTER_PRINTF_FORMAT
      "] to ladd of -lconst \n", s->optDetailString(), node))
      {
      ...
      if (performTransformation(s->comp(), "%sReduced lsub with negated second child in node [" POINTER_PRINTF_FORMAT
      "] to ladd\n", s->optDetailString(), node))
      {
         TR::Node *newSecondChild = secondChild->getFirstChild();
         TR::Node::recreate(node, TR::ladd);
      ...
      if (performTransformation(s->comp(), "%sReduced lsub with negated first child in node [" POINTER_PRINTF_FORMAT
      "] to lneg of ladd\n", s->optDetailString(), node))
      {
         TR::Node *newFirstChild = firstChild->getFirstChild();
   ...
   {
      if (performTransformation(s->comp(),
                                "%sReduced lsub of bitwise complement and lconst -1 in node ["
         POINTER_PRINTF_FORMAT
...
// 184251
   ...
   if (!node->nodeRequiresConditionCodes() &&
      ...
      if (((mask + 1) & mask) == 0 && // mask + 1 power of two?
          performTransformation(s->comp(), "%sNext lower pwr of 2 using land ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
...
TR::Node *dsubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
      foldDoubleConstant(node,
                         TR::Compiler->arith.doubleSubtractDouble(firstChild->getDouble(), secondChild->getDouble()),
                         s);
      return node;
   }
...
TR::Node *lmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1) {
      ...
      TR::Node *lrChild = firstChild->getSecondChild();
      if (lrChild->getOpCodeValue() == TR::lconst &&
          performTransformation(s->comp(), "%sDistributed lmul with lconst over lsub or ladd with lconst in node ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
...
TR::Node *fmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (bothChildrenNegated) {
      if (performTransformation(s->comp(), "%sTransforming [" POINTER_PRINTF_FORMAT
      "] (-A)*(-B) -> A*B\n", s->optDetailString(), node))
      { // remove negates
...
TR::Node *dmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
      foldDoubleConstant(node,
                         TR::Compiler->arith.doubleMultiplyDouble(firstChild->getDouble(), secondChild->getDouble()),
                         s);
      return node;
   }
...
TR::Node *fdivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (secondChild->getOpCode().isLoadConst() &&
      ...
      if (firstChild->getOpCode().isLoadConst()) {
         foldFloatConstant(node, TR::Compiler->arith.floatDivideFloat(firstChild->getFloat(), secondChild->getFloat()),
                           s);
         return node;
      }
...
TR::Node *ddivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (secondChild->getOpCode().isLoadConst()) {
      if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
         foldDoubleConstant(node,
                            TR::Compiler->arith.doubleDivideDouble(firstChild->getDouble(), secondChild->getDouble()),
                            s);
         return node;
      }
...
TR::Node *inegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (opCode == TR::ineg) {
      ...
      if (performTransformation(s->comp(), "%sReduced ineg with isub child in node [" POINTER_PRINTF_FORMAT
      "] to isub\n", s->optDetailString(), node))
      {
         TR::Node::recreate(node, TR::isub);
         node->setNumChildren(2);
      ...
      if (shiftBy == 63 &&
          performTransformation(s->comp(), "%sReplaced ineg of lushr by 63 with lshr node ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
...
TR::Node *fnegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCodeValue() == TR::fneg) {
      ...
      if (child2Change &&child2Change->getReferenceCount() == 1 && performTransformation(s->comp(), "%sTransforming ["
      POINTER_PRINTF_FORMAT
      "] -(-A op B) -> A op B (op=*,/,%%)\n", s->optDetailString(), node))
      {
         // remove neg
      ...
      if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) &&
          ...
          performTransformation(s->comp(), "%sTransforming ["
         POINTER_PRINTF_FORMAT
      "] -(-A +/- B) -> -((A*1)+/-B)\n", s->optDetailString(), node))
      {
...
TR::Node *ilfdabsSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (child->isNonNegative() &&
       ...
       performTransformation(s->comp(), "%sSimplify abs of non-negative child at ["
      POINTER_PRINTF_FORMAT
   "]\n", s->optDetailString(), node))
   {
      return s->replaceNodeWithChild(node, child, s->_curTree, block);
            ...
            performFlagBasedTransformation &&
            performTransformation(s->comp(), "%sSimplify abs of non-positive child at ["
      POINTER_PRINTF_FORMAT
   "]\n", s->optDetailString(), node))
   {
      ...
      return s->simplify(node, block);
            ...
            performTransformation(s->comp(), "%sSimplify abs of abs/neg child at ["
      POINTER_PRINTF_FORMAT
   "]\n", s->optDetailString(), node))
   {
...
TR::Node *lshlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (secondChild->getOpCode().isLoadConst()) {
      ...
      //
      performTransformation(s->comp(), "%sCanonicalize long left shift by constant in node ["
      POINTER_PRINTF_FORMAT
...
TR::Node *iushrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (secondChild->getOpCodeValue() == TR::iconst &&
      ...
      if (constGrandChild->getOpCodeValue() == TR::iconst &&
         ...
         //
         if (performTransformation(s->comp(), "%sReduced left shift followed by iushr in node [" POINTER_PRINTF_FORMAT
         "] to iand with mask\n", s->optDetailString(), node))
         {
            TR::Node::recreate(node, TR::iand);
...
TR::Node *lushrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (secondChild->getOpCodeValue() == TR::iconst &&
      ...
      if (constGrandChild->getOpCodeValue() == TR::lconst &&
         ...
         //
         if (performTransformation(s->comp(), "%sReduced left shift followed by lushr in node [" POINTER_PRINTF_FORMAT
         "] to land with mask\n", s->optDetailString(), node))
         {
...
TR::Node *landSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getReferenceCount() == 1) {
      if (isBitwiseLongComplement(firstChild) &&
         ...
         if (performTransformation(s->comp(),
                                   "%sReduced land with two complemented children in node ["
            POINTER_PRINTF_FORMAT
         "] to complemented lor\n", s->optDetailString(), node))
         {
         ...
         bool foundZeroExtension = false;
         if (maskValue == 255 && firstChildOp == TR::b2i) {
            if (performTransformation(s->comp(), "%sReduced land with lconst 255 in node [" POINTER_PRINTF_FORMAT
            "] to bu2l\n", s->optDetailString(), node))
            {
               TR::Node::recreate(node, TR::bu2l);
               foundZeroExtension = true;
            ...
            if (performTransformation(s->comp(), "%sReduced land with lconst 65536 in node [" POINTER_PRINTF_FORMAT
            "] to su2l\n", s->optDetailString(), node))
            {
               TR::Node::recreate(node, TR::su2l);
               foundZeroExtension = true;
            ...
            if (performTransformation(s->comp(), "%sReduced land with lconst 0xffffffff in node [" POINTER_PRINTF_FORMAT
            "] to iu2l\n", s->optDetailString(), node))
            {
               TR::Node::recreate(node, TR::iu2l);
               foundZeroExtension = true;
   ...
   if (node->getOpCodeValue() == TR::land && secondChild->getOpCodeValue() == TR::lconst &&
      ...
      if (secondChild->isHighWordZero() && ((int32_t) secondChild->getLongIntLow()) > 0) {
         ...
         if (firstChildOp == TR::iu2l &&
             performTransformation(s->comp(), "%sReduced land with lconst and iu2l child in node ["
            POINTER_PRINTF_FORMAT
         "] to iand\n", s->optDetailString(), node))
         {
...
TR::Node *removeRedundantIntegralOrPattern1(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (!performTransformation(s->comp(), "%sRemove redundant %s 0x%llx [" POINTER_PRINTF_FORMAT
   "] under %s 0x%llx ["
   POINTER_PRINTF_FORMAT
   "]\n", s->optDetailString(),
...
TR::Node *removeRedundantIntegralOrPattern2(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   dumpOptDetails(s->comp(), "%sCreate new zero extension conversion %s ["
   POINTER_PRINTF_FORMAT
   "] of childOr child %s ["
   POINTER_PRINTF_FORMAT
   "]\n",
...
TR::Node *iorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getReferenceCount() == 1) {
      if (isBitwiseIntComplement(firstChild) &&
          ...
          isBitwiseIntComplement(secondChild)) {
         if (performTransformation(s->comp(),
                                   "%sReduced ior with two complemented children in node ["
            POINTER_PRINTF_FORMAT
         "] to complemented iand\n", s->optDetailString(), node))
         {
            TR::Node *andNode = TR::Node::create(TR::iand, 2, firstChild->getFirstChild(),
         ...
         if (lrChild->getOpCodeValue() == TR::iconst) {
            if (secondChildOp == TR::iconst) {
               ...
               if ((((~and_const) | or_const) == or_const) &&
                   performTransformation(s->comp(), "%sFound ior of iconst with iand of x and iconst in node ["
                  POINTER_PRINTF_FORMAT
               "]\n", s->optDetailString(), node))
               {
                  node->setAndIncChild(0, firstChild->getFirstChild());
...
TR::Node *lorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getReferenceCount() == 1) {
      if (isBitwiseLongComplement(firstChild) &&
          ...
          isBitwiseLongComplement(secondChild)) {
         if (performTransformation(s->comp(),
                                   "%sReduced lor with two complemented children in node ["
            POINTER_PRINTF_FORMAT
         "] to complemented land\n", s->optDetailString(), node))
         {
            TR::Node *andNode = TR::Node::create(TR::land, 2, firstChild->getFirstChild(),
                                                 secondChild->getFirstChild());
            TR::Node *constNode = firstChild->getSecondChild();
            TR::Node::recreate(node, TR::lxor);
            node->setAndIncChild(0, andNode);
   ...
   if (node->getOpCodeValue() == TR::lor && secondChild->getOpCodeValue() == TR::lconst &&
      ...
      if (secondChild->isHighWordZero() && ((int32_t) secondChild->getLongIntLow()) > 0) {
         ...
         if (firstChildOp == TR::iu2l &&
             performTransformation(s->comp(), "%sReduced lor with lconst and iu2l child in node ["
            POINTER_PRINTF_FORMAT
         "] to ior\n", s->optDetailString(), node))
         {
            TR::Node *constChild = NULL;
...
TR::Node *borSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCode().isAnd() &&
       ...
       performTransformation(s->comp(), "%sReplacing bor ["
      POINTER_PRINTF_FORMAT
   "] child with band child ["
   POINTER_PRINTF_FORMAT
   "] \n",
...
TR::Node *lxorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (node->getOpCodeValue() == TR::lxor && secondChild->getOpCodeValue() == TR::lconst &&
      ...
      if (secondChild->isHighWordZero() && ((int32_t) secondChild->getLongIntLow()) > 0) {
         ...
         if (firstChildOp == TR::iu2l &&
             performTransformation(s->comp(), "%sReduced lxor with lconst and iu2l child in node ["
            POINTER_PRINTF_FORMAT
         "] to ixor\n", s->optDetailString(), node))
         {
            TR::Node *constChild = NULL;
...
inline TR::Node *sqrtSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (child->getOpCode().isLoadConst() &&
       performTransformation(s->comp(), "%sSimplify sqrt of const child at ["
      POINTER_PRINTF_FORMAT
   "]\n", s->optDetailString(), node))
   {
...
TR::Node *i2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getReferenceCount() == 1) {
      ...
      if (childOp == TR::su2i) {
         if (performTransformation(s->comp(), "%sReduced i2l with su2i child in node [" POINTER_PRINTF_FORMAT
         "] to su2l\n", s->optDetailString(), node))
         {
            TR::Node::recreate(node, TR::su2l);
         ...
         if (performTransformation(s->comp(), "%sReduced i2l with s2i child in node [" POINTER_PRINTF_FORMAT
         "] to s2l\n", s->optDetailString(), node))
         {
         ...
         if (performTransformation(s->comp(), "%sReduced i2l with b2i child in node [" POINTER_PRINTF_FORMAT
         "] to b2l\n", s->optDetailString(), node))
         {
   ...
   if (firstChild->getOpCodeValue() == TR::l2i &&
      ...
      uint32_t shiftBy = firstFirst->getSecondChild()->getInt();
      if (shiftBy >= 57 &&
          performTransformation(s->comp(), "%sRemove i2l/l2i from lshr node ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
         TR::Node::recreate(node, TR::lshr);
...
TR::Node *iu2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getReferenceCount() == 1) {
      bool foundFoldableConversion = false;
      if (childOp == TR::su2i) {
         if (performTransformation(s->comp(), "%sReduced iu2l with su2i child in node [" POINTER_PRINTF_FORMAT
         "] to su2l\n", s->optDetailString(), node))
         {
            TR::Node::recreate(node, TR::su2l);
            foundFoldableConversion = true;
         ...
         if (performTransformation(s->comp(), "%sReduced iu2l with bu2i child in node [" POINTER_PRINTF_FORMAT
         "] to bu2l\n", s->optDetailString(), node))
         {
            TR::Node::recreate(node, TR::bu2l);
            foundFoldableConversion = true;
...
TR::Node *l2aSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if ((firstChild->getOpCodeValue() == TR::lsub || firstChild->getOpCodeValue() == TR::ladd) &&
      ...
      TR::ILOpCodes addressAddOp = ((constChild->getReferenceCount() == 1) &&
                                    // is firstChild the only parent of constChild?
...
TR::Node *bu2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCodeValue() == TR::i2b &&
      ...
      if (((val & 0xFFUL) == 0UL) &&
          performTransformation(s->comp(), "%sReplacing bu2i ["
         POINTER_PRINTF_FORMAT
      "] with i2b child ["
      POINTER_PRINTF_FORMAT
      "] of iand ["
            ...
            s->optDetailString(), node, firstChild, firstChild->getFirstChild(), val, TR::getMaxUnsigned<TR::Int8>()))
      {
         ...
         return node;
               ...
               performTransformation(s->comp(), "%sRemove bu2i ["
         POINTER_PRINTF_FORMAT
      "] with i2b child ["
      POINTER_PRINTF_FORMAT
      "] of iand ["
   ...
   if (firstChild->getOpCodeValue() == TR::l2b &&
      ...
      if (shiftBy >= 56 &&
          performTransformation(s->comp(), "%sReplace bu2i/l2b of lushr with l2i node ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
...
TR::Node *bu2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCodeValue() == TR::i2b &&
      ...
      uint64_t val = firstChild->getFirstChild()->getSecondChild()->get64bitIntegralValueAsUnsigned();
      if (((val & 0xFFUL) == 0UL) &&
          performTransformation(s->comp(), "%sReplacing bu2l ["
         POINTER_PRINTF_FORMAT
      "] with i2b child ["
      POINTER_PRINTF_FORMAT
      "] of iand ["
            ...
            s->optDetailString(), node, firstChild, firstChild->getFirstChild(), val, TR::getMaxUnsigned<TR::Int8>()))
      {
         ...
         return node;
               ...
               performTransformation(s->comp(), "%sReplace bu2l ["
         POINTER_PRINTF_FORMAT
      "] with i2b child ["
      POINTER_PRINTF_FORMAT
      "] of iand ["
            ...
            s->optDetailString(), node, firstChild, firstChild->getFirstChild(), val, TR::getMaxUnsigned<TR::Int8>()))
      {
         ...
         node->setChild(0, grandChild);
              ...
              (firstChild->getFirstChild()->getOpCodeValue() == TR::icmpeq ||
               ...
               firstChild->getFirstChild()->getOpCodeValue() == TR::lcmpeq ||
               firstChild->getFirstChild()->getOpCodeValue() == TR::lcmpne) &&
              performTransformation(s->comp(), "%sRemove bu2l ["
      POINTER_PRINTF_FORMAT
   "] with i2b child ["
   POINTER_PRINTF_FORMAT
   "] of compare ["
   ...
   "]\n", s->optDetailString(), node, firstChild, firstChild->getFirstChild()))
   {
      ...
      uint32_t shiftBy = firstChild->getFirstChild()->getSecondChild()->getInt();
      if (shiftBy >= 56 &&
          performTransformation(s->comp(), "%sReplace bu2l/l2b of lushr with lushr node ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
...
TR::Node *s2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getReferenceCount() == 1) {
      bool foundFoldableConversion = false;
      if (childOp == TR::bu2s) {
         if (performTransformation(s->comp(), "%sReduced s2i with bu2s child in node [" POINTER_PRINTF_FORMAT
         "] to bu2i\n", s->optDetailString(), node))
         {
            TR::Node::recreate(node, TR::bu2i);
            foundFoldableConversion = true;
         ...
         if (performTransformation(s->comp(), "%sReduced s2i with b2s child in node [" POINTER_PRINTF_FORMAT
         "] to b2i\n", s->optDetailString(), node))
         {
...
TR::Node *s2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getReferenceCount() == 1) {
      bool foundFoldableConversion = false;
      if (childOp == TR::bu2s) {
         if (performTransformation(s->comp(), "%sReduced s2l with bu2s child in node [" POINTER_PRINTF_FORMAT
         "] to bu2l\n", s->optDetailString(), node))
         {
            TR::Node::recreate(node, TR::bu2l);
            foundFoldableConversion = true;
         ...
         if (performTransformation(s->comp(), "%sReduced s2l with b2s child in node [" POINTER_PRINTF_FORMAT
         "] to b2l\n", s->optDetailString(), node))
         {
            TR::Node::recreate(node, TR::b2l);
            foundFoldableConversion = true;
   ...
   if (firstChild->getReferenceCount() == 1) {
      ...
      if (childOp == TR::i2s) {
         ...
         if (firstGrandChild->getReferenceCount() == 1 && firstGrandChild->getOpCodeValue() == node->getOpCodeValue()) {
            if (performTransformation(s->comp(), "%sReduced su2i node [" POINTER_PRINTF_FORMAT
            "] and i2s child ["
            POINTER_PRINTF_FORMAT
...
arithUnderSignedAndUnsignedCompareHandler(TR::Node *opNode, TR::Node *secondChild, TR::Simplifier *s, TR::Node *node,
   ...
   if (!isSigned) {
      ...
      //Assuming oldUConst1+oldUConst2>unsignedMax --> resulting in overflow,
      //We can identify such overflow from
      //oldUConst2>unsignedMax-oldUConst1 without causing overflow in the calculation itself
...
TR::Node *ificmpltSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (node->getOpCodeValue() == TR::ificmplt) {
      if (firstChild->getOpCode().isLoadConst() &&
          conditionalBranchFold((originalFirst->getInt() < originalSecond->getInt()), node, firstChild, secondChild,
                                block, s))
         return node;
...
TR::Node *ifCmpWithEqualitySimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (!disableFoldIfSet && node->getOpCodeValue() == TR::ifbcmpeq) {
      if (secondChild->getOpCodeValue() == TR::bconst &&
         ...
         if (op != TR::BadILOp &&
             performTransformation(s->comp(), "%sFolding ifbcmpeq of bconst 0 to boolean compare at node ["
            POINTER_PRINTF_FORMAT
...
TR::Node *icmpeqSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCodeValue() == TR::iand &&
      ...
      if ((val & (val - 1)) == 0 && val == mask &&
          performTransformation(s->comp(), "%s Changing icmpeq (x&2**c) to 2**c node ["
         POINTER_PRINTF_FORMAT
      "] to iand\n", s->optDetailString(), node))
      {
...
TR::Node *lcmpeqSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCodeValue() == TR::land &&
      ...
      if ((val & (val - 1)) == 0 && val == mask &&
          performTransformation(s->comp(), "%s Changing lcmpeq (x&2**c) to 2**c node ["
         POINTER_PRINTF_FORMAT
      "] to land\n", s->optDetailString(), node))
      {
         uint32_t shiftBy = trailingZeroes(mask);
...
TR::Node *lcmpneSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (secondChild->getOpCode().isLoadConst() &&
      ...
      if (firstSecond->getOpCodeValue() == TR::lshl &&
          ...
          performTransformation(s->comp(), "%slcmpne of x & (1 << y) to 0 opt node ["
         POINTER_PRINTF_FORMAT
      "]\n", s->optDetailString(), node))
      {
         TR::Node::recreate(node, TR::iand);
         ...
         if ((val & (val - 1)) == 0 && // power of two?
             performTransformation(s->comp(), "%slcmpne of (x & 2**c) to 0 node ["
            POINTER_PRINTF_FORMAT
         "]\n", s->optDetailString(), node))
         {
...
TR::Node *lcmpltSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (secondChild->getOpCode().isLoadConst() &&
       ...
       performTransformation(s->comp(), "%sReplace lcmplt to 0 with lushr node ["
      POINTER_PRINTF_FORMAT
   "]\n", s->optDetailString(), node))
   {
      TR::Node *shiftBy = TR::Node::create(node, TR::iconst, 0);
...
TR::Node *switchSimplifier(TR::Node *node, TR::Block *block, bool isTableSwitch, TR::Simplifier *s) {
   ...
   if (firstChild->getOpCode().isLoadConst()) {
      ...
      if (node->getOpCode().getOpCodeValue() == TR::lookup && NULL != shiftNode && shiftNode->getOpCode().isShift()) {
         ...
         if (NULL != shiftAmtNode && shiftAmtNode->getOpCode().isLoadConst()) {
            ...
            if (!canTransform || !performTransformation(s->comp(), "%sRemoving shift node [" POINTER_PRINTF_FORMAT
            "] from lookup node ["
            POINTER_PRINTF_FORMAT
            ...
            for (int caseIndex = 0; caseIndex < numCases; caseIndex++) {
               ...
               caseNode->setCaseConstant(
                     isLeftShift ? (caseNode->getCaseConstant() >> shiftAmount) : (caseNode->getCaseConstant()
                           << shiftAmount));
            }
         }
      }
   ...
   if (convertToGoto) {
      ...
      //
      if (!performTransformation(s->comp(), "%sChanging node [" POINTER_PRINTF_FORMAT
      ...
      for (int childIndex = 1; childIndex < numChildren; childIndex++)//loop from default to last case
      {
         ...
         if (destInst->getOpCodeValue() == TR::Goto) {
            ...
            if (!performTransformation(s->comp(), "%sRedirecting switch [" POINTER_PRINTF_FORMAT
            ...
            if (cfg) {
               ...
               int otherChildIndex;
               for (otherChildIndex = 1; otherChildIndex <
                                         numChildren; ++otherChildIndex)//Look for another case/default that leads to destBlock.
      ...
      if (prevTree) {
         ...
         if ((prevNode->getOpCodeValue() == TR::ificmpeq) ||
            ...
            if (firstChild->getOpCodeValue() == TR::instanceof &&
                ...
                performTransformation(s->comp(), "%sRemoving checkcast node ["
               POINTER_PRINTF_FORMAT
            "]\n", s->optDetailString(), node))
            {
      ...
      if (lengthChild == boundChild ||
         ...
         if (indexChild->getOpCodeValue() == TR::isub) {
            ...
            if (boundChild->getOpCodeValue() == TR::iadd &&
               ...
               if (performTransformation(s->comp(),
                                         "%sRemoved arraycopy bndchk with arrayLength index in node [%s] when bound is sum of length and index arraylengths\n",
                                         s->optDetailString(), node->getName(s->getDebug()))) {
                  s->removeNode(node, s->_curTree);
                  return NULL;
               }
...
TR::Node *arraysetSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
   ...
   if (fill->getOpCode().isLoadConst() && fill->getDataType() == TR::Int64) {
      ...
      if ((fillVal & 0x0FFFFFFFFL) == ((fillVal >> 32) & 0x0FFFFFFFFL) &&
          performTransformation(s->comp(), "%sTransform large fill arrayset to 4byte fill arrayset ["
         POINTER_PRINTF_FORMAT
      "]\n",
            s->optDetailString(), node))
      {
